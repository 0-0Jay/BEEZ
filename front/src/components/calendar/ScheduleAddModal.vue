  ...
  <Dialog :visible="visible" @update:visible="$emit('update:visible', $event)" modal header="일정 추가" :style="{ width: '400px' }" :closable="true" :draggable="false">
    <div class="flex flex-col gap-4 py-2">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <InputText :value="event?.title" placeholder="일정 제목을 입력하세요" class="w-full" @input="updateField('title', $event.target.value)" @keyup.enter="handleSave" />
      </div>
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="flex flex-col gap-1.5">
          ...
          <DatePicker :value="event?.start" dateFormat="yy-mm-dd" placeholder="시작일" class="w-full" showIcon @update:modelValue="updateField('start', $event)" />
        </div>
        <div class="flex flex-col gap-1.5">
          <label class="text-xs font-semibold text-gray-500">종료일</label>
          <DatePicker :value="event?.end" dateFormat="yy-mm-dd" placeholder="종료일" class="w-full" showIcon @update:modelValue="updateField('end', $event)" />
        </div>
      </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <Button label="취소" severity="secondary" text size="small" @click="$emit('update:visible', false)" />
        <Button label="저장" size="small" :disabled="!event?.title || !event?.start" @click="handleSave" />
      </div>
    </template>
  </Dialog>
</template>
