...
<template>
  <div class="border border-[#5B6E96] rounded-xl p-6 h-[600px] overflow-y-auto">
    <div class="flex justify-between items-center border-b border-[#C7C7C2] pb-3 mb-4">
      <h2 class="text-sm font-medium text-[#1A1816]">구성원 추가</h2>
    ...
    <div v-if="groupStore.selectedMembers.length === 0" class="flex flex-col items-center justify-center h-48">
      ...
      <p class="text-xs text-[#6B6B63]">추가된 구성원이 없습니다.</p>
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-2">
      <div v-for="member in groupStore.selectedMembers" :key="member.id" class="border border-[#E0E0DA] rounded-lg p-3 relative">
        ...
        <p class="text-xs font-medium text-[#1A1816] mb-1">이름 : {{ member.name }}</p>
        <p class="text-xs text-[#6B6B63]">이메일 : {{ member.email }}</p>
      </div>
    </div>
  </div>
