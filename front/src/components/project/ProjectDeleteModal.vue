...
const isMatched = computed(() => inputValue.value === props.identifier);
  ...
  <Dialog :visible="visible" :modal="true" :draggable="false" :closable="true" header="삭제 확인" :style="{ width: '480px' }" @update:visible="(val) => emit('update:visible', val)">
    <div class="flex flex-col gap-5">
      <p class="text-base text-stone-600 leading-relaxed">
        정말 삭제하시려면 아래에
        <strong class="text-stone-900 font-semibold">{{ identifier }}</strong
        >을(를) 입력해주세요.
      </p>

      <div class="flex flex-col gap-1.5">
        <InputText v-model="inputValue" :placeholder="identifier" :invalid="showError" class="w-full" />
        <p v-if="showError" class="text-sm text-red-500">식별자를 정확하게 입력해 주세요.</p>
